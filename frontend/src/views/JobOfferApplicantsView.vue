<template>
    <div class="py-4">
        <template v-if="route.name === 'applicant-directory-job-offer'">
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <select v-model="activeFilter" class="input input-bordered input-sm w-36">
                            ...
                            <option value="jobOfferDate">Job Offer Date</option>
                        ...
                        <template v-else-if="activeFilter === 'jobOfferDate'">
                            From
                            <input v-model="jobOfferFrom" type="date" class="input input-bordered input-sm w-46" placeholder="From" />
                            To
                            <input v-model="jobOfferTo" type="date" class="input input-bordered input-sm w-46" placeholder="To" />
                        </template>
                        <template v-else-if="activeFilter === 'onboardingDate'">
                            From
                            <input v-model="onboardingFrom" type="date" class="input input-bordered input-sm w-46" placeholder="From" />
                            To
                            <input v-model="onboardingTo" type="date" class="input input-bordered input-sm w-46" placeholder="To" />
                        </template>
                    </div>
                </div>
                    ...
                    <table class="min-w-full dark:bg-gray-800">
                        <thead>
                            <tr class="bg-green-400 text-white text-xs">
                                ...
                                <th class="px-4 py-2 font-semibold">Job Offer Date</th>
                                <th class="px-4 py-2 font-semibold">Onboarding Date</th>
                            </tr>
                        </thead>
                        <tbody>
...
const dateFrom = ref('')
const dateTo = ref('')
const jobOfferFrom = ref('')
const jobOfferTo = ref('')
const onboardingFrom = ref('')
const onboardingTo = ref('')
const filteredApplicants = computed(() => {
    ...
    } else if (activeFilter.value === 'jobOfferDate') {
        filtered = filtered.filter(a => {
            if (jobOfferFrom.value && new Date(a.jobOfferDate) < new Date(jobOfferFrom.value)) return false
            if (jobOfferTo.value && new Date(a.jobOfferDate) > new Date(jobOfferTo.value)) return false
            return true
        })
    } else if (activeFilter.value === 'onboardingDate') {
        filtered = filtered.filter(a => {
            if (onboardingFrom.value && new Date(a.onboardingDate) < new Date(onboardingFrom.value)) return false
            if (onboardingTo.value && new Date(a.onboardingDate) > new Date(onboardingTo.value)) return false
            return true
        })
    }
    return filtered
