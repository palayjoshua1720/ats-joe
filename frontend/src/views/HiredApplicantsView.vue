        ...
        <div class="bg-white rounded shadow p-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-2">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <template v-else-if="activeFilter === 'dateHired'">
                        From
                        <input v-model="dateHiredFrom" type="date" class="input input-bordered input-sm w-46" placeholder="From" />
                        To
                        <input v-model="dateHiredTo" type="date" class="input input-bordered input-sm w-46" placeholder="To" />
                    </template>
                </div>
            </div>
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="bg-green-700 text-white text-xs">
                            ...
                            <th class="px-4 py-2 font-semibold">Date Hired</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(applicant, i) in filteredApplicants" :key="i" :class="[
                            ...
                            <td class="px-4 py-2 text-xs text-center">{{ applicant.dateHired }}</td>
                        </tr>
                        <tr v-if="filteredApplicants.length === 0">
                            <td colspan="8" class="text-center text-gray-400 py-6">No applicants found.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="flex items-center justify-between mt-4">
                ...
                <nav class="inline-flex -space-x-px">
                    <button class="px-2 py-1 text-xs text-gray-500 border border-gray-300 rounded-l hover:bg-green-200 bg-white transition-colors">&lt;</button>
                    ...
                    <button class="px-2 py-1 text-xs text-gray-500 border border-gray-300 rounded-r hover:bg-green-200 bg-white transition-colors">&gt;</button>
                </nav>
            </div>
        </div>
    </div>
...
const applicants = [
    { position: 'Php Developer', name: 'David Patrick Wilson', dateApplied: 'June 25, 2023', email: 'dwilson@gmail.com', contact: '09878986534', salary: 21000, dateHired: '' },
]
...
const dateFrom = ref('')
const dateTo = ref('')
const dateHiredFrom = ref('')
const dateHiredTo = ref('')
const filteredApplicants = computed(() => {
    ...
    } else if (activeFilter.value === 'dateHired') {
        filtered = filtered.filter(a => {
            if (dateHiredFrom.value && new Date(a.dateHired) < new Date(dateHiredFrom.value)) return false
            if (dateHiredTo.value && new Date(a.dateHired) > new Date(dateHiredTo.value)) return false
            return true
        })
    }
    return filtered
